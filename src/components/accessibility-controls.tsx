...
type Props = {
  /** When true, render panel only (no floating launcher). Used by SiteUtilityDock. */
  embedded?: boolean;
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
};

export function AccessibilityControls({ embedded = false, open: controlledOpen, onOpenChange }: Props) {
  ...
  const panel = open || embedded ? (
    ...
    >
      ...
      <div className="mt-3 grid gap-3 text-sm">
        ...
        <button
          type="button"
          className="min-h-[44px] rounded-[var(--radius-ui)] border border-border bg-background px-3 py-2 text-sm font-semibold text-heading"
  ...
  return (
    ...
    >
      <button
        type="button"
        className="accessibility-button min-h-[44px] rounded-[var(--radius-ui)] border border-border bg-card px-3 py-2 text-sm font-semibold text-heading shadow-[var(--shadow-soft)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-focus-ring)]"
